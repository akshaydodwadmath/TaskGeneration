...
class KarelForSynthesisParser(Parser):
    ...
    def p_prog(self, p):
        '''prog : DEF RUN M_LBRACE action3 action3 stmt_fixed  M_RBRACE'''
        #'''prog : DEF RUN M_LBRACE action3 NEWLINE stmt_fixed NEWLINE action3 M_RBRACE'''
        #'''prog : DEF RUN M_LBRACE action3 M_RBRACE DEF RUN M_LBRACE stmt_fixed M_RBRACE'''
        action3, stmt, action3 = p[4], p[5], p[6]

        @self.callout
        def fn():
            action3(); action3(); stmt(); 
        p[0] = fn
