...
class KarelWithCurlyParser(Parser):
    ...
    def p_prog(self, p):
       '''prog : DEF RUN LPAREN RPAREN LBRACE action3 RBRACE NEWLINE DEF RUN LPAREN RPAREN LBRACE stmt_fixed RBRACE NEWLINE DEF RUN LPAREN RPAREN LBRACE action3 RBRACE'''
       #'''prog :  subprog'''
       # action = p[6]
       subprog = p[1]
       def fn():
    ...
    def p_fullprog(self, p):
        '''fullprog : DEF RUN LPAREN RPAREN LBRACE action3 stmt action3 RBRACE'''
        action3, stmt, action3 = p[6], p[8] , p[10]
        def fn():
            action3(); stmt(); action3();
        p[0] = fn
  
    def p_subprog(self, p):
        '''subprog : DEF RUN LPAREN RPAREN LBRACE action3 RBRACE'''
        action3, stmt, action3 = p[6], p[8] , p[10]
        def fn():
            action3(); stmt(); action3();
        p[0] = fn

    def p_stmt_fixed(self, p):
        '''stmt_fixed : stmt
        '''
        stmt = p[1]
        p[0] = lambda: stmt()
    ...
    def p_action3(self, p):
        ...
        action, action, action = p[1], p[2] , p[3]
        def fn():
