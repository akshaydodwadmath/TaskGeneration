...
class MultiIOProgramDecoder(nn.Module):
    ...
    def forward(self, tgt_inp_sequences, in_src_seq, tgt_encoder_vector,seq_len,
       ...
       # tgt_emb = self.embedding_output(torch.argmax(tgt_encoder_vector, dim=1).reshape(-1,1))
        ...
        embedding_matrix =  self.embedding_output(F.one_hot(torch.arange(0, 16) % 16).cuda())
        
        tgt_emb = torch.matmul(tgt_encoder_vector.unsqueeze(1) , embedding_matrix)
...
class TransformerModel(nn.Module):
    ...
    def forward(self, Temp, src: Tensor) -> Tensor:
        """
        Args:
       ...
       # probs = nn.Softmax(dim=1)(output)
        if((numpy.random.choice(numpy.arange(1, 5), p=[0.25, 0.25, 0.25, 0.25])) == 1):
            probs = torch.Tensor(16, 16).fill_(0).cuda()
            ...
            #probs = F.gumbel_softmax(output, tau=0.1, hard=True)
            #Temp = 1
            if(Temp > 0 ):
                probs = nn.Softmax(dim=1)(output/ Temp)
            else:
                probs = torch.Tensor(16, 16).fill_(0).cuda()
                index = torch.argmax(output, dim=1)
                count = 0 
...
class CodeType2Code(nn.Module):
    ...
    def forward(self, tgt_inp_sequences, in_src_seq, out_tgt_seq):
       ...
       # io_embedding = self.encoder(input_grids, output_grids)
        tgt_encoder_vector,output = self.trnsfrmrEncoder(self.Temp, out_tgt_seq )
        if(in_src_seq[0].item() == 5):
            if(self.Temp > 0):
                self.Temp -= 0.01
