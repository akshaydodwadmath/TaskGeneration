...
class World:
    ...
    ## Also check the grid size and walls.
    def is_same(self, other):
        ...
        if other.heroDir == "any":
            ...
            if self.heroDir != other.heroDir: return False
        if self.crashed != other.crashed: return False
        if not self.equalWalls(other): return False
        return self.equalMakers(other)
    ...
    # Are the walls the same in these two worlds?
    def equalWalls(self, other):
        return (self.blocked == other.blocked).all()
    ...
    # TODO remove that. It's redundunt
    def toCarpedm(self):
        WALL_CHAR = '#'
        ...
        state = np.zeros((self.rows, self.cols), dtype=np.int8)
        zero_state = np.tile(
        ...
        x, y = self.heroCol, self.heroRow
        agentdir = self.heroDir
        ...
        for r in range(self.rows):
            for c in range(self.cols):
                if self.blocked[r][c] == 1:
                    state[r][c][4] = 1
                elif int(self.markers[r][c]) > 0:
                    if int(self.markers[r][c]) > 9:
                        raise Exception(
                            f"Number of markers are over the limit: {int(self.markers[r][c])}")
                    ## I guess this would accept maximum of 9
                    state[r][c][int(self.markers[r][c]) + 5] = 1
        return state
    ...
    def toTSVString(self):
        ...
        #         header = tag + f'_{ind}' + '\t' + "".join([str(i) + '\t' for i in range(1, 2 + self.cols + 1)])
        #         worldStr += header + '\n'
        topWall = ('#' + '\t') * (self.cols + 2 - 1) + '#'
        worldStr += topWall + '\n'
        #         worldStr += ('#' + '\t')*(self.cols - 1) + '#' + '\n'
        for r in range(self.rows - 1, -1, -1):
            rowStr = ''  # str(self.rows - r + 1) + '\t'
            rowStr += '#' + '\t'
            for c in range(0, self.cols):
                if self.blocked[r][c] == 1:
                    rowStr += '#'
        ...
        bottomWall = ('#' + '\t') * (self.cols + 2 - 1) + '#'
        worldStr += bottomWall + '\n'

        #         worldStr += f'agentloc_{ind}' + '\t' + f'(col={self.heroCol + 1},row={self.rows - self.heroRow + 1})' + '\n'
        #         worldStr += f'agentdir_{ind}' + '\t' + heroDir
    ...
    # the third grid is the beeper locations. The fourth grid is karel_emulator.
    # The rows of the grid are interchanged so the visualization of
    # karel_emulator will match the matrices. If the padding
    ...
    def toTensor(self, padding):
        ...
        # fourth matri is the location of karel_emulator
        tensor[3][self.rows - self.heroRow - 1][
            self.heroCol] = self.getHeroDirValue()  # TODO add any option
        return tensor

    def toPytorchTensor(self, padding):
        tensor = torch.FloatTensor(16, padding, padding).zero_()
        # Put in the hero
        dir = self.getHeroDirValue()
        if dir != 'any':
            tensor[dir - 1][self.heroRow + 1][self.heroCol + 1] = 1
        else:
            tensor[:4][:, self.heroRow + 1, self.heroCol + 1] = 1
    ...
    # TODO Add a way to represent any direction as a valid direction. Only Postgrid can contain any direction
    ...
    def getHeroDirValue(self):
        ...
        if (self.heroDir == 'any'): return "any"
        raise ("invalid dir")
        ...
        for r in range(self.rows - 1, -1, -1):
            ...
            for c in range(0, self.cols):
                if self.blocked[r][c] == 1:
                    ...
                    rowStr += f"{Tcolors.OKGREEN}{self.getHeroChar()}{Tcolors.ENDC}"
                elif self.markers[r][c] > 0:
                    numMarkers = int(self.markers[r][c])
                    if numMarkers > 9:
