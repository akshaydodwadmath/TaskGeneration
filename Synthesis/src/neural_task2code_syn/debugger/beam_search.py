...
BeamSearchResult = collections.namedtuple('BeamSearchResult',
                                          ['sequence', 'total_log_prob', 'log_probs',
                                           'log_probs_torch'])
...
def Calculate_prob_idx_token_with_penalty(cuda, step, sizes, total_log_probs,
                                          finished_penalty, lengths_penalty, factor):
    """
    ...
    if step > 0:
        ...
        lengths_to_add = Variable(
            tt.FloatTensor(batch_size * actual_beam_size * logit_size).fill_(0)).view(
            batch_size, actual_beam_size, logit_size)
        add_mask = (1 - finished_penalty)
        lengths_to_add = add_mask.repeat(1, logit_size).view(batch_size,
                                                             actual_beam_size,
                                                             logit_size) * lengths_to_add

        new_prediction_lengths = lengths_to_add + lengths_penalty.repeat(1,
                                                                         logit_size).view(
            batch_size, actual_beam_size, logit_size).float()
...
def beam_search_(batch_size,
    ...
    # as the beam size.
    b_idx = Variable(
        torch.arange(0, batch_size, out=torch.LongTensor()).unsqueeze(1).repeat(1,
                                                                                beam_size).view(
            -1))

    finished_penalty = Variable(tt.FloatTensor(batch_size * beam_size).fill_(0)).view(
        batch_size, beam_size)
    lengths_penalty = Variable(tt.LongTensor(batch_size * beam_size).fill_(0)).view(
        batch_size, beam_size)
    ...
    for step in range(max_decoder_length):
        ...
        if use_length_penalty:
            prev_probs, indices, prev_tokens, finished_penalty, lengths_penalty = Calculate_prob_idx_token_with_penalty(
                cuda, step, sizes, total_log_probs, finished_penalty, lengths_penalty,
                factor)
            ...
            if beam_size != actual_beam_size:
                prev_probs = torch.cat([prev_probs,
                                        -10000 + prev_probs[:, 0].unsqueeze(1).repeat(1,
                                                                                      beam_size - actual_beam_size)],
                                       dim=1)
                indices = torch.cat([indices, indices[:, 0].unsqueeze(1).repeat(1,
                                                                                beam_size - actual_beam_size)],
                                    dim=1)
        ...
        for batch_id in range(batch_size):
            ...
            for idx in range(beam_size):
                ...
                if token == 1:  # 1 == END
                    if differentiable:
                        log_probs_torch = prev_result[batch_id][
                                              kidx].log_probs_torch + [
                                              log_probs[batch_id, kidx, token]]
                    else:
                    ...
                    finished[batch_id].append(BeamSearchResult(
                        ...
                        log_probs=prev_result[batch_id][kidx].log_probs + [
                            log_probs_np[batch_id, kidx, token]],
                        log_probs_torch=log_probs_torch))
                    result[batch_id].append(
                        BeamSearchResult(sequence=[], log_probs=[], total_log_prob=0,
                                         log_probs_torch=[]))
    ...
    if not return_beam_search_result:
        for batch_id in range(batch_size):
            finished[batch_id] = [(x.total_log_prob, x.sequence) for x in finished[
                batch_id]]
