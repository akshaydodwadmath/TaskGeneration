...
class PlaceholderVocab(object):
    ...
    def reset(self):
        self._last_idx = 0
        self._indicies = range(len(self._vocab),
                               len(self._vocab) + self._num_placeholders)
...
def tokenize_code_line(line):
    ...
    for i in range(len(line)):
        if unicodedata.category(line[i]) in ('Zs', 'Cc', 'Cf'):
            ...
            new_type = 'L'
            if prev_type not in ('D', 'L') and not (
                    prev_type == 'P' and last_token[-1] == '_'):
                push = True
            ...
            if prev_type != 'P' or not is_prefix(last_token, line[i]):
                push = True
            if line[i] == '_' and (
                    prev_type in ('D', 'L') or prev_type == 'P' and last_token[
                -1] == '_'):
                push = False
            if i + 1 < len(line) and line[i] == '.' and prev_type == 'D' and \
                    unicodedata.category(line[i + 1])[0] == 'N':
                push = False
        else:
            print("WTF:", line[i], unicodedata.category(line[i]))
        if push:
