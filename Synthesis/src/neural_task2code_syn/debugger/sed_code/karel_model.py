...
from . import beam_search, karel, edit, data
from . import prepare_spec
from .base import BaseCodeModel
...
def encode_grids_and_outputs(batch, code, vocab):
    inp_grids = [entry['in_grids'] for entry in batch]
    out_grids = [entry['out_grids'] for entry in batch]

    input_grids = torch.stack(inp_grids)
    output_grids = torch.stack(out_grids)
    code_seqs = prepare_spec.lists_padding_to_tensor(
        [item for item in code], vocab.stoi, False)
...
class KarelLGRLModel(BaseKarelModel):
    ...
    def compute_loss(self, input_tuple):
        ...
        if self.args.cuda:
            ...
            code_seqs = code_seqs.cuda(non_blocking=True)
    ...
    def batch_processor(self, for_eval):
        return KarelLGRLBatchProcessor(self.vocab, for_eval,
                                       self.args.train_policy_gradient_loss)
...
class KarelLGRLRefineModel(BaseKarelModel):
    ...
    def compute_loss(self, input_tuple):
        ...
        if self.args.train_policy_gradient_loss:
            return self.calculate_policy_gradient_loss(input_grids, io_embed,
    ...
    def calculate_policy_gradient_loss(self, input_grids, io_embed, orig_examples,
                                       ref_code, ref_code_memory,
                                       ref_trace_memory):
        ...
        for logit_beam, code_beam, example in zip(sequences, output_code,
            ...
            for i, (logits, code) in enumerate(zip(logit_beam, code_beam)):
                ...
                all_logits.append(
                    torch.sum(torch.cat([x.view(1) for x in logits.log_probs_torch])))
                run_cases = None  # lambda tests: executor.evaluate_code(code,
                #  example.schema.args, tests, self.executor.execute)
...
class PackedTrace(collections.namedtuple('PackedTrace', ('actions',
                                                         'action_code_indices', 'conds',
                                                         'cond_code_indices',
                                                         'interleave_indices'))):
    def cuda(self, non_blocking=False):
        actions = maybe_cuda(self.actions, non_blocking)
        action_code_indices = maybe_cuda(self.action_code_indices, non_blocking)
        conds = maybe_cuda(self.conds, non_blocking)
        cond_code_indices = maybe_cuda(self.cond_code_indices, non_blocking)
...
class KarelLGRLRefineBatchProcessor(object):
    ...
    def __call__(self, batch, code, tgt_seq):
        input_grids, output_grids, code_seqs = encode_grids_and_outputs(
            batch, [tgt_seq], self.vocab)
        ...
        #     dec_data = self.compute_edit_ops([code], [tgt_seq], ref_code,
        #                                       self.return_edits)
        # else:
        dec_data = None
        ...
        if self.args.use_ref_orig:
            orig_examples = prepare_spec.numpy_to_tensor(prepare_spec.lists_to_numpy(
                [('<S>',) + item + ('</S>',) for item in
                 [code]], self.vocab.stoi, -1), False, False)
    ...
    def compute_edit_ops(self, code, tgt_seq, ref_code, return_edits=False):
        ...
        for batch_idx, item in enumerate(code):
            edit_ops = list(
                edit.compute_edit_ops(item,
                                      tgt_seq[batch_idx], self.vocab.stoi))
            dest_iter = itertools.chain(['<s>'], tgt_seq[batch_idx])

            # Op = <s>, emb location, last token = <s>
            source_locs, ops, values = [list(x) for x in zip(*edit_ops)]
            source_locs.append(len(item))
            ops = [0] + ops
            values = [None] + values
    ...
    def prepare_traces_grids(self, batch):
        ...
        ref_trace_grids = lists_to_packed_sequence(grids_lists, (16, 18, 18),
