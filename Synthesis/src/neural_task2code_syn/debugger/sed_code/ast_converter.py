...
class AstParser(object):
    ...
    def __parse_block(self, t, start, final_delim):
        ...
        while True:
            if index >= len(t):
                self.__raise_exception(t, index, "Missing final delimiter '{0}'".format(
                    final_delim))
    ...
    def __parse_command(self, t, index):
        ...
        if command == 'REPEAT':
            ...
            if m is None:
                self.__raise_exception(t, index + 1, "Not a repeat token: {0}".format(
                    num_times_string))
    ...
    def __parse_cond(self, t, index):
        ...
        if token == 'not':
            ...
            self.__raise_exception(t, index,
                                   "Unexpected conditional token: {0}".format(token))
    ...
    def __get_token(self, t, index):
        if index < 0 or index >= len(t):
            self.__raise_exception(t, index,
                                   "Index out of range: {0} (length = {1})".format(
                                       index, len(t)))
        return t[index]
    ...
    def __raise_exception(self, t, index, error_message):
        msg = "Error parsing tokens: '{0}'. Error message: {1}".format(" ".join(t),
                                                                       error_message)
