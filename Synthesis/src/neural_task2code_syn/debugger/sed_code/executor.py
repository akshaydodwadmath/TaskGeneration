...
import numpy as np
import pylru
from src.agent.debugger_agent.sed_code.parser_for_synthesis import \
    KarelForSynthesisParser
from src.agent.debugger_agent.sed_code.utils import Timeout, KarelSyntaxError
...
ACTIONS = {x.lower() for x in
           ('move', 'turnRight', 'turnLeft', 'pickMarker', 'putMarker')}
REPEATS = {'repeat'}
...
class KarelExecutor(object):
    ...
    def gather_coverage(self, datum, result):
        try:
            ...
            coverage = [dict(action=[0] * program_length, repeat=[0] * program_length,
                             branch=[[0, 0]] * program_length)
                        for _ in range(len(datum.input_tests))]

            for test_idx, test in enumerate(datum.input_tests):
                events = self.execute(result['output'], None, test['input'],
                                      record_trace=True).trace.events
                for event in events:
