...
from src.agent.debugger_agent.sed_code import saver, data
...
class InferenceResult(object):
    ...
    def dovetail(inference_results):
        ...
        assert all(res.info.keys() == {'trees_checked', 'candidates'} for res in
                   inference_results)
        candidates = []
        for i in count():
            done = True
        ...
        return InferenceResult(code_tree=code_tree, code_sequence=code_sequence,
                               info=dict(trees_checked=trees_checked,
