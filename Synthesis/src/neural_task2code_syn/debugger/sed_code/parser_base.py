...
from src.agent.debugger_agent.sed_code import yacc
from src.agent.debugger_agent.sed_code.karel_runtime import KarelRuntime
from src.agent.debugger_agent.sed_code.utils import get_rng, str2bool
...
class Parser(object):
    ...
    def parse(self, code, **kwargs):
        ...
        if isinstance(code, (list, tuple)):
            return self.yacc.parse(None,
                                   tokenfunc=self.token_list_to_tokenfunc(code),
                                   **kwargs)
        else:
    ...
    def random_tokens(self, start_token="prog", depth=0, stmt_min_depth=2,
                      stmt_max_depth=5, **kwargs):
...
def parser_prompt(parser):
    ...
    arg_parser.add_argument('--world', type=str, default=None,
                            help='Path to world text file')
    arg_parser.add_argument('--world_height', type=int, default=8,
                            help='Height of square grid world')
    arg_parser.add_argument('--world_width', type=int, default=8,
