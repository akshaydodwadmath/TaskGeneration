...
def get_arg_parser(title, mode):
    ...
    parser.add_argument('--iterative-search-start-with-beams', action='store_true',
                        help="start with the beams from the original model")
    parser.add_argument('--iterative-search-step-limit', type=int, default=5)
    parser.add_argument('--iterative-search-use-overfit-model',
                        help="k=v,k=v... pairs of data to put into the overfit model")
    ...
    if mode == 'train':
        ...
        train_group.add_argument('--eval_every_n', type=int,
        ...
        train_group.add_argument('--read-code', dest='read_code', action='store_true',
                                 default=False)
        train_group.add_argument('--read-text', dest='read_text', action='store_true',
                                 default=True)
        train_group.add_argument('--skip-text', dest='read_text', action='store_false')
        train_group.add_argument('--read-io', dest='read_io', action='store_true',
                                 default=False)
        ...
        train_group.add_argument('--karel-hidden-size', type=int, default=256,
                                 help="the hidden size to use in LSTM, etc. Changing this value from the default only works for a vanilla model (--model_type karel-lgrl-ref --karel-trace-enc none --karel-refine-dec edit)")
        ...
        eval_group.add_argument('--hide-example-info', action='store_true',
                                default=False)
    ...
    infer_group.add_argument('--karel-file-ref-train',
                             help='json file containing a list of dictionaries with keys '
                                  ...
                                  'expected')
    infer_group.add_argument('--karel-file-ref-val',
                             help='see help for --karel-ref-file-train')
    infer_group.add_argument('--karel-file-ref-train-balancing',
                             choices=['equal-count', 'none'], default='equal-count',
    ...
    # TODO: it requires this
    # infer_group.add_argument('--karel-trace-enc', default='none')
    ...
    infer_group.add_argument('--ensemble-mode', choices=['none', 'dovetail'],
                             default='none')
