...
from src.agent.debugger_agent.sed_code import beam_search, data, prepare_spec, base
...
class LGRLTaskEncoder(nn.Module):
    ...
    def forward(self, input_grid, output_grid):
        ...
        input_grid = input_grid.contiguous().view(-1, 16, 18, 18)
        output_grid = output_grid.contiguous().view(-1, 16, 18, 18)
...
class GridEncoder(nn.Module):
    def __init__(self, num_grids, channels, output_size=256):
        ...
        self.initial_conv = nn.Conv2d(
            in_channels=num_grids * 16, out_channels=channels, kernel_size=3, padding=1)
...
class TraceLSTM(nn.Module):
    ...
    def forward(self, traces):
        output, _ = self.lstm(traces.ps,
                              lstm_init(traces.ps.data.is_cuda, self.num_layers * 2,
...
def get_edges(trace_events, inp_lengths, inp_itv, trace_lengths, trace_itv,
    ...
    edges += generate_sequence_edges(sequence_edge_limit, 'tt', trace_itv,
                                     trace_lengths)
...
class SummarizationTrace(nn.Module, ABC):
    ...
    def forward(self, inp_embed, trace_embed, trace_events, program_lengths):
        all_sum_traces = self.summarize_trace_per_token(trace_embed, trace_events,
                                                        program_lengths)
    ...
    def summarize_trace_per_token(self, trace_embed, trace_events, program_lengths):
        all_sum_traces = []
        for batch_idx, (item, prog_len) in enumerate(
                zip(trace_events.spans, program_lengths)):
            # traces_Per_program_token[i] == (trace indices, weights)
            trace_indices = collections.defaultdict(
                list)  # ZERO INDEXED, unlike in Spans
            ...
            for i in range(prog_len):
                if not trace_indices[i]:
                    ...
                    continue
                traces_for_token = trace_embed.select(
                    *trace_overall_index(batch_idx, trace_indices[i]))
                weights_for_this = Variable(torch.FloatTensor(weights[i]))
                if traces_for_token.is_cuda:
                    weights_for_this = weights_for_this.cuda()
                sum_traces.append(
                    self.summarize_traces(traces_for_token, weights_for_this))
...
def trace_overall_index(batch_idx, test_time_indices):
    ...
    return [batch_idx * 5 + test for test, _ in test_time_indices], [time for _, time in
                                                                     test_time_indices]
...
class CodeEncoderAlternative(nn.Module):
    def __init__(self, vocab_size, args):
        ...
        self.encoder = nn.LSTM(input_size=256, hidden_size=256, num_layers=2,
...
class TraceEncoder(nn.Module):
    ...
    def forward(self, code_enc, traces):
        ...
        # traces: tuple of
        # - grids: PackedSequencePlus, batch x seq x 16 (channels) x 18 x 18
...
class TimeConvTraceEncoder(TraceEncoder):
    ...
    def forward(self, code_enc, traces_grids, traces_events, cag_interleave):
        ...
        # grids: batch x seq length x 16 (channels) x 18 x 18
        ...
        # grids: batch x 16 x seq length x 18 x 18
        ...
        if self.interleave_events:
            action_embs = traces_events.actions.apply(
                lambda d: self.action_code_proj(
                    code_enc.mem.ps.data[
                        traces_events.action_code_indices]) * self.success_emb(d[:, 1])
            )
...
class LGRLSeqRefineDecoder(nn.Module):
    def __init__(self, vocab_size, args):
        ...
        if code_enc and self.use_code_memory:
            self.code_attention = None  # SimpleSDPAttention(256, 256 * 2)
        if trace_enc and self.use_trace_memory:
            self.trace_attention = None  # SimpleSDPAttention(256, 256 * 2)
...
class LGRLSeqRefineEditDecoder(nn.Module):
    def __init__(self, vocab_size, args):
        ...
        if self.use_code_attn:
            self.code_attention = None  # SimpleSDPAttention(hs, hs * 2)
    ...
    def init_state(self, ref_code_memory, ref_trace_memory,
        ...
        return LGRLRefineEditDecoderState(
            ...
            finished,
            context,
            *lstm_init(self._cuda, 2, self.args.karel_hidden_size, batch_size,
                       pairs_per_example))
...
class PackedSequenceAttention(nn.Module):
    ...
    def forward(self, ps, extra):
        ...
        for batch_idx, length in enumerate(ps.orig_lengths()):
            ...
            values.append(
                (logits_for_this.softmax(axis=0) * data_for_this).sum(0).unsqueeze(0))
