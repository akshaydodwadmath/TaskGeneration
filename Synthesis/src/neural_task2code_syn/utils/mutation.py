...
CONDS.extend({'type': 'not', 'condition': cond} for cond in CONDS[:3])
CONDS_MASKED_PROBS = {
    n: masked_uniform(len(CONDS), i)
    for i, n in
    enumerate(('frontIsClear', 'leftIsClear', 'rightIsClear', 'markersPresent',
...
REPEAT_COUNTS = [i for i in range(2, 11)]
...
def random_singular_block(rng):
    ...
    if type_ == 'repeat':
        ...
        return {'type': type_, 'condition': rng.choice(CONDS)}
...
class TreeIndex(object):
    def __init__(self, tree):
        ...
        while queue:
            ...
            if 'run' in node:
                bodies = [BodyInfo(node, 'run', node['run'])]
                if address[0]:
            ...
            if 'condition' in node or 'times' in node:
                self.cond_locs.append(node)
            if 'run' not in node and node['type'] in ('if', 'while'):
...
def mutate(tree, probs=None, rng=None):
    ...
    if choice == ADD_ACTION:
        ...
        if 'condition' in node:
            node['condition'] = rng.choice(
                CONDS,
                p=CONDS_MASKED_PROBS[node['condition']['type'] + node['condition'].get(
                    'condition', {}).get('type', '')])
        elif 'repeat' in node:
...
def mutate_n(tree, count, probs=None, rng=None, allow_in_place=False):
    ...
    previous_seqs = set([tuple(traverse_pre_order(tree, False))])
    for i in range(count):
        found = False
        for _ in range(1000):
            tree = copy.deepcopy(tree)
            mutate(tree, probs, rng)
            new_seq = tuple(traverse_pre_order(tree, False))
...
if __name__ == '__main__':
    code = {"run": [{"type": "turnRight"},
                    {"body": [{"type": "move"},
                              {"type": "pickMarker"}],
                     "times": 2, "type": "repeat"}]}
    print(mutate_n(code, 5))
